...
def normalize_to_unit_interval(
    ...
    *,
    ...
    """
    Robust min-max scale a score Series to [0,1] using percentile clipping.
    Parameters
    ----------
    s : pd.Series
        Raw score vector (e.g., LOF negative_outlier_factor_).
    flip_sign : bool
        If True, multiply by -1 first (needed for LOF so larger = more anomalous).
    q_low : float
        Lower percentile bound for scaling (default = 0.01).
    q_high : float
        Upper percentile bound for scaling (default = 0.99).
    Returns
    -------
    pd.Series
        Scaled scores in [0,1], same index as input.
        Larger values = more anomalous (if flip_sign=True for LOF).
    """
    s = s.copy()
    if flip_sign:
        s = -s  # LOF: more negative = more anomalous
    p_low = s.quantile(q_low)
    p_high = s.quantile(q_high)
    scaled = ((s - p_low) / (p_high - p_low)).clip(0, 1)
    scaled.name = s.name
    return scaled